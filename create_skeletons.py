...
def write_exercise_templates(strng):
    for chunk in strng.split("\n//\n"):
        chunk = chunk.strip()
        major, minor = map(int, re.search(r"Exercise (\d+)-(\d+)\.", chunk).groups([1, 2]))
        filename = "exercise_{}-{:02d}.c".format(major, minor)
        if not os.path.exists(filename):
            with open(filename, "w") as fp:
                fp.write(chunk)
                fp.write(main_function)
...
// into visible escape sequences like and as it copies the string t to s. Use a
